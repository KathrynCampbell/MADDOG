...
sequences <- seqinr::read.fasta(file = (paste(folder, "/assignment/", folder, "_WGS_assignment.fasta", sep = "")))

ref_align<-seqinr::read.fasta("inst/extdata/References/RABV/reference_aligned.fasta")
...
for (i in 1:length(test_seqs)){
  ...
  if (!is.na(test)){
    if (up==down) {
      ...
      if ((test_seqs[i]+(x-1)) >= length(ref_align)) {
        intup<-0
      } else {
        intup<-alignment[test_seqs[i]+(x-1),]
      }
      if ((test_seqs[i]-(y-1)) <= 1) {
        intdown<-0
      }else{
...
write.csv(test_seq_assignment, file = (paste(folder, "/assignment/", folder, "_assignment.csv", sep = "")), row.names = F)
