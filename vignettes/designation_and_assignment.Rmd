...
The designation requires some initial steps to be undertaken on the sequences first. They need to be aligned (it's reccommended to use MAFFT and use the FFT-NS-2 algorithm), and then a tree created including bootstrapping (reccomended dusing IQTREE2 with model selection and 100 bootstrap replicates). The ancestral sequences then need to be reconstructed (reccommend using Treetime ancestral).
...
``` {r assignment}
assignments<-MADDOG::assign_lineages(sequences, "Cosmo_WGS"); assignments
```
